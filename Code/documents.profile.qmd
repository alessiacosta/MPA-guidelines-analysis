...
# bar plot 
jurisdiction_plot <- 
  ggplot(jurisdiction1, aes(x=Scale, y=percent)) +
...
categories <- 
  ggplot(fig1_long, aes(x = Category, y = Percentage, fill = Variable)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("MPA objectives" = "#FDBF2D", "Design guidelines" = "#3B7CA3")) +
  guides(fill = guide_legend(title = NULL), 
         x=guide_axis(minor.ticks= TRUE)) +
  labs(x = "Categories", 
       #y = "Percentage of documents (%)", # presentation figure
       title = "  " ) +
  coord_cartesian(clip = "off") +
  ...
  theme(axis.text.x = element_text(), 
        ...
        axis.ticks.length = unit(4, "pt"),                         # make sure ticks are visible
        panel.border = element_rect(colour = "black", fill=NA, linewidth=0.25)) + # adds a border around the plot 
  annotate("text", x=-Inf, y=Inf, label="C", hjust=1.5, vjust=0.2, size=6, fontface="bold", family='serif') +
  annotate("text", x = 4.85, y = 2, label = "0", size = 3)
#png("../Results/presentation.figure.png", width = 2600, height = 1600, res = 300)
categories
#dev.off()
```
...
# Plotting the geography of the documents
```{r}
# setting levels 
profile_geo <- profile |> 
  mutate(Geography = factor(Geography, level = c(
    "Africa", 
    "Asia", 
    "Australia/Oceania",
    "Europe", 
...
# wrapping text
profile_geo$Geography <- profile_geo$Geography |> 
  fct_recode(
    "Africa" = "Africa",
    "Asia" = "Asia",
...
# producing plot
geography <- 
  ggplot(profile_geo, aes(x = Geography)) +
  geom_bar(fill="#4A97A0") + 
  theme_minimal(base_family = "Times") + 
  coord_cartesian(clip = "off") +
  theme(
    axis.ticks = element_line(color = "black"), 
    axis.ticks.length = unit(4, "pt"),              
    panel.border = element_rect(colour = "black", fill=NA, linewidth=0.25)) +
  labs(x = "Documents geography", 
       y = " Number of documents")
# exporting the plot 
png("../Results/geography.png", width = 2300, height = 1600, res = 300)
geography
dev.off()
