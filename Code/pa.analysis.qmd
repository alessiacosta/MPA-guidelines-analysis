...
# Governace objectives/guidelines

```{r}
# Convert the data to a wide format
create_heatmap <- function(data, title, show_legend=FALSE) {
data_wide <- dcast(data, Citation ~ Guideline, value.var = "Presence")
data_melted1 <- melt(data_wide, id.vars = "Citation", variable.name = "Guideline", value.name = "Presence")

# Calculate the total presence for each Citation
citation_presence <- data_melted1 |> 
  group_by(Citation) |> 
  summarise(total_presence = sum(Presence)) |> 
  arrange(desc(total_presence))

# Reorder Citation based on total presence
data_melted <- data_melted1 |> 
  mutate(Citation = factor(Citation, levels = citation_presence$Citation))

# Create the presence/absence heatmap with reordered y-axis
heatmap <- ggplot(data_melted, aes(x = Citation, y = reorder(Guideline, Presence), fill = factor(Presence))) +
  geom_tile(color = "white") +
  scale_fill_manual(
    values = c("0" = "white", "1" = "#3B7CA3"), 
...
#Create heatmaps for each dataset
heatmap1 <- create_heatmap(community_gov, "A - Community-centred conservation governance", show_legend = FALSE)
heatmap2 <- create_heatmap(legal_structures, "B - Effective legal structures", show_legend = TRUE)

#Combine plots into one figure
combined_plot <- (heatmap1 + heatmap2) +
  plot_layout(guides='collect') +
  plot_annotation(
    theme=theme(plot.title=element_text(face="bold", size=15, margin=margin(b=20)))
  )

#Export teh combined plot into a png file
png("../Results/FigS3.png", width=3200, height=2700, res=300)
print(combined_plot)
dev.off()
```